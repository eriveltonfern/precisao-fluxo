...
const CityPage = () => {
  ...
  useEffect(() => {
    if (city) document.title = `Desentupidora em ${city.name} | ${s?.company_name || "Desentupidora Precisão"} 24h`;
  }, [city, s]);
    ...
    <div className="min-h-screen">
      ...
      <section className="py-16 md:py-20">
        <div className="container">
          <h2 className="font-display text-2xl font-extrabold text-foreground md:text-3xl mb-8">Bairros atendidos em {city.name}</h2>
          ...
          <div className="flex flex-wrap gap-3">
            {allCities?.filter((c) => c.slug !== slug).map((c) => (
              <Link key={c.slug} to={`/desentupidora/${c.slug}`}
                className="rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium text-foreground shadow-sm hover:border-primary/30">{c.name}</Link>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-primary-dark text-center">
        <div className="container">
          <h2 className="font-display text-2xl font-extrabold text-white md:text-3xl">Precisa de desentupidora em {city.name}?</h2>
